# AGENTS.md — smalls-automation/uat-golden-20260510

## Stack
- Language: C# (.NET 9)
- Solution: `uat-golden.sln` at the repo root
- Main project: `src/uat-golden.Api/`
- Tests: `tests/uat-golden.Tests/` — xUnit + Moq (pre-created, already references src project)

## Commands
- Build: `dotnet build uat-golden.sln` (run from repo root — always name the .sln)
- Test: `dotnet test uat-golden.sln`
- Format: `dotnet format`
- New migration: `dotnet ef migrations add {Name}` (run from repo root)

## Project structure
```
uat-golden.sln          ← solution file at repo root — do NOT recreate
src/uat-golden.Api/     ← the only project directory — all source goes here
  uat-golden.Api.csproj ← project file — do NOT recreate
...
**CRITICAL:** All source files go inside `src/uat-golden.Api/` — never directly in `src/`.
...
4. `dotnet build uat-golden.sln` must exit 0 before you finish
